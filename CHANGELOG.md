...
- `influxdb3-plugin-cli`: human-mode `package` success output shortens artifact and derived-index paths to CWD-relative form when the target is a descendant of the working directory; otherwise the absolute path is preserved. JSON-mode payloads continue to carry absolute paths.
