...
fn all_valid_manifests_parse_and_fields_match_snapshot() {
    ...
    for (name, contents) in pairs {
        let manifest = Manifest::parse_toml(&contents).unwrap_or_else(|e| panic!("{name}: {e}"));
...
fn all_valid_indexes_parse_and_fields_match_snapshot() {
    ...
    for (name, contents) in pairs {
        let index = Index::parse_json(&contents).unwrap_or_else(|e| panic!("{name}: {e}"));
...
fn all_invalid_indexes_report_expected_errors() {
    ...
    for (name, contents) in pairs {
        let errors =
            Index::parse_json(&contents).expect_err(&format!("{name}: was expected to fail parse"));
