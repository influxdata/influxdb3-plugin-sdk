...
- A Rust toolchain for the public go-live install path.
- The `influxdb3-plugin` CLI.
...
The `triggers` array must match the functions implemented by the Python file. See [Manifest format](../../02-reference/manifest.md) for the complete schema.
...
See [Index format](../../02-reference/registry-index.md) for the index schema.
...
curl -fsSLo .github/workflows/publish.yml \
  https://raw.githubusercontent.com/influxdata/influxdb3-plugin-sdk/main/docs/src/03-templates/github-actions/publish.yml
```
...
The template walkthrough is [GitHub Actions publish workflow](../../03-templates/github-actions/).
...
Next: [GitHub Actions publish workflow](../../03-templates/github-actions/).
